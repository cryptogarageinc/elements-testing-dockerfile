...
RUN export BITCOIN_TARBALL=bitcoin-${BITCOIN_VERSION}-aarch64-linux-gnu.tar.gz \
...
RUN export ELEMENTS_TARBALL=elements-elements-${ELEMENTS_VERSION}-arm-linux-gnueabihf.tar.gz \
...
RUN export CMAKE_DIR_NAME=cmake-${CMAKE_VERSION}-linux-aarch64 \
